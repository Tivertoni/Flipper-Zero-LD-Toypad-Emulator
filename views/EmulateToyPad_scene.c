...
static void ldtoypad_scene_emulate_draw_render_callback(Canvas *canvas, void *context)
{
    ...
    if ((model->selected_minifigure_index > 0 || model->selected_vehicle_index > 0) &&
        model->connected)
    {
        int id = model->selected_minifigure_index > 0 ? model->selected_minifigure_index : model->selected_vehicle_index;
        ...
        if (is_vehicle)
        {
            ...
            if (id < min_vehicle_id)
            {
                id = min_vehicle_id;
            }
            else if (id > max_vehicle_id)
            {
                id = max_vehicle_id;
            }
        }
        ...
        {
            model->selected_minifiguire_index = 0;
            if (id < min_character_id)
            {
                id = min_character_id;
            }
    ...
    for (int i = 0; i < numBoxes; i++)
    {
        if (boxInfo[i].isFilled)
        {
            ...
            {
                if (token->id)
                { // Only minifigures have an id, vehicles have no id, but is stored in the "token[180]"
...
LDToyPadSceneEmulate *ldtoypad_scene_emulate_alloc(LDToyPadApp *new_app)
{
    ...
    if (emulator == NULL)
        emulator = malloc(sizeof(ToyPadEmu));
